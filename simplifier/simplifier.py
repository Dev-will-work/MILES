...
def suitable_id_candidate(w, c, user_id):
    ...
    not_complex = zfreq(c, data_map[user_id].lang) > zfreq(w, data_map[user_id].lang)
    not_stopword = c not in safe_get_stop_words(data_map[user_id].lang) and c.isalpha()

    return not_equal_stem and not_stem_len and not_morph_deriv and not_stopword and not_complex
    
def simplify_id_token(tokens, i, user_id):
    ...
    candidates = [c for c in candidates if suitable_id_candidate(tokens[i], c, user_id)][::-1]
    complex_ranked = sorted(candidates, key = lambda c: zfreq(c, data_map[user_id].lang))

    if data_map[user_id].embeddings:
        # If WEs have been loaded, include cosine and apsynp metrics.
        cosine_ranked = sorted(candidates, key = lambda c: cosine_id_sim(tokens[i], c, user_id))
        apsynp_ranked = sorted(candidates, key = lambda c: apsyn_id_sim(tokens[i], c, user_id))
...
def simplify_id_text(text, user_id, bold_highlight=False):
    ...
    for i in range(len(tokens_copy)):
        ...
        if suitable_complex_id_word(tokens_copy[i], user_id):
            candidates = simplify_id_token(tokens_copy, i, user_id)
